...
namespace FileSystemWeb.Areas.DeskAdmin.Controllers
{
    ...
    public class AssignFileController : Controller
    {
        ...
        public IActionResult AllCompain()
        {
            return View("~/Areas/DeskAdmin/Views/AssignFile/AllComplainList.cshtml");
        }
        ...
        public IActionResult GetAllIssueFileList(string fsFileName, int? Status, int? sort_column, string sort_order, int? pg, int? size)
        {
            try
            {
                string lsSearch = string.Empty;
                ...
                List<IssueFileListResult> loIssueFileListResult = new List<IssueFileListResult>();
                loIssueFileListResult = moUnitOfWork.IssueFileHistoryRepository.GetIssueFileListByDeskAdmin(fsFileName == null ? fsFileName : fsFileName.Trim(), sort_column, sort_order, pg.Value, size.Value, null, Convert.ToInt32(User.FindFirst(SessionConstant.DepartmentId).Value.ToString()), null, Status);
                dynamic loModel = new ExpandoObject();
                loModel.GetIssueFileList = loIssueFileListResult;
                if (loIssueFileListResult.Count > 0)
                {
                ...
                loModel.Pagination = PaginationService.getPagination(liTotalRecords, pg.Value, size.Value, liStartIndex, liEndIndex);
                return PartialView("~/Areas/DeskAdmin/Views/AssignFile/_AllComplainList.cshtml", loModel);
            }
            catch (Exception ex)
            {
                return RedirectToAction("Index", "Error");
