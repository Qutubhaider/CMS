...
function sort(fsSortColumn, element) {
    if ($(element).closest('th').hasClass("sorting_asc")) {
        ...
        getCategoryData(lsPageNo, fsSortColumn, "desc");
    }
    else {
        $(element).closest('th').removeClass("sorting_desc").addClass("sorting_asc");
        lsSortOrder = 'asc';
        lsSortCol = fsSortColumn;
        getCategoryData(lsPageNo, fsSortColumn, "asc");
    }
}
function getCategoryData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    ...
    loadMyRequest(msGetCategoryDataList, "GET", loData, getCategoryDataSuccess, getCategoryDataError);
    $('#preloader').hide();
}

function getCategoryDataError(foResponse) {
    hideLoading();
}

function getCategoryDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#CategoryList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#CategoryList').find("table tbody").empty();
        $('#CategoryList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
