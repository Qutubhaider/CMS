...
function sort(fsSortColumn, element) {
    if ($(element).closest('th').hasClass("sorting_asc")) {
        ...
        getComplainData(lsPageNo, fsSortColumn, "desc");
    }
    else {
        $(element).closest('th').removeClass("sorting_desc").addClass("sorting_asc");
        lsSortOrder = 'asc';
        lsSortCol = fsSortColumn;
        getComplainData(lsPageNo, fsSortColumn, "asc");
    }
}
function getComplainData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();
    var lsComplainTitle = $('#txtComplain').val();
    var liStatus = '';
    ...
    loData.complain = lsComplainTitle;
    ...
    loadMyRequest(msGetComplainDataList, "GET", loData, getComplainDataSuccess, getComplainDataError);
    $('#preloader').hide();
}

function getComplainDataError(foResponse) {
    hideLoading();
}

function getComplainDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#ComplainList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#ComplainList').find("table tbody").empty();
        $('#ComplainList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
...
function GeneratePageListpanel(fsvalue, ffName) {
    ...
    for (liCounter = 0; liCounter <= liListPanelSize + 1; liCounter++) {
        ...
        if (liCounter == 0) {
            ...
            if (liPageNo <= liTotalpages) {
                lsHTML = lsHTML + '<li id="pagging_' + liPageNo + '" class="paginate_button page-item "><a href="javascript:onclick=getCategoryData(' + liPageNo + ');" data-dt-idx="3" tabindex="0" class="page-link" id=aLinkPageNo' + liPageNo + '>' + liPageNo + '</a></li>';
            }
        }
    }
    lsHTML = lsHTML + "</ul>";
